...
/** How tall the bottom chrome is. iOS is the only platform still on a system
 * tab bar, and its height is a platform constant rather than something the
 * root layout can measure from outside the tabs. Everywhere else DOOEY draws
 * its own island, which floats at the same height on the web and on Android. */
const ISLAND = 50;
const IOS_TABS = 49;
...
export function useDockTop(docked = true): number {
  ...
  if (Platform.OS === "ios") return insets.bottom + IOS_TABS;
  return Math.max(16, insets.bottom) + ISLAND;
}
