...
export function useUpdatePack() {
  ...
  return useMutation({
    ...
    onMutate: ({ id, doodles }) => {
      qc.setQueryData<DoodlePack[]>(packKeys.list, (ps) =>
        ps?.map((p) => (p.id === id ? { ...p, doodles } : p)),
      );
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: packKeys.list }),
  });
 ...
 * normalized space: x spans 0–100 of the bounding box and y keeps the same
 * scale, so `aspect` (w/h) reproduces the drawing exactly at any width. */
