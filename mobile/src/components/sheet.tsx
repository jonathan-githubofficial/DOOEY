...
const settle = LinearTransition.springify().stiffness(300).damping(32);
 ...
 * at the button and settles open, with `transformOrigin` pinned to the corner
 * the button sits at. Barely any overshoot — it opens, it doesn't spring. */
const unfold: EntryExitAnimationFunction = () => {
  ...
  return {
    ...
    animations: {
      ...
      transform: [{ scale: withSpring(1, { stiffness: 320, damping: 30 }) }],
    },
  };
};
...
function Sheet({ spec, seq }: { spec: SheetSpec; seq: number }) {
  ...
  return (
    <Animated.View
      ...
      style={[styles.backdrop, WEB && styles.backdropCentred]}
    >
      <Pressable accessibilityLabel="Dismiss" style={StyleSheet.absoluteFill} onPress={closeSheet} />
      <Animated.View
        entering={WEB ? FadeIn.duration(160) : SlideInDown.springify().stiffness(300).damping(30)}
        layout={settle}
        style={[
...
const styles = StyleSheet.create({
  backdrop: { flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(20,16,12,0.35)" },
